...
$base = Join-Path (Split-Path -Parent $PSScriptRoot) 'WeatherData_AWS/interpolation_batch/fullnet/elevation_hour_effect'
$out = Join-Path $base 'altitude_span_cases_20260905'
...
# 사용자 승인 고도 배치 기준. 선정에 성능값은 사용하지 않는다.
$targetElevations = @(50.0,300.0,600.0,900.0,1200.0,1500.0)
$selection = @()
for ($i=0; $i -lt $targetElevations.Count; $i++) {
    $targetElevation = $targetElevations[$i]
    $chosen = $stations | Sort-Object @{Expression={[math]::Abs([double]$_.elev_m-$targetElevation)}},@{Expression={[double]$_.elev_m}},@{Expression={[int]$_.STN}} | Select-Object -First 1
    $selection += [pscustomobject]@{GROUP=($i+1);TARGET_ELEV_M=$targetElevation;STN=$chosen.STN;NAME=$chosen.'지점명';ELEV_M=[double]$chosen.elev_m;DISTANCE_TO_TARGET_M=[math]::Abs([double]$chosen.elev_m-$targetElevation);COVERAGE_DAYS=$chosen.COVERAGE_DAYS;COVERAGE_MONTHS=$chosen.COVERAGE_MONTHS}
}
if (@($selection.STN|Select-Object -Unique).Count -ne 6) {throw '고도별 최근접 선택 중복: 임의 재선정 금지'}
...
if ($rows.Count -ne 144 -or @($selection.STN|Select-Object -Unique).Count -ne 6) {throw '최종 크기 불일치'}
...
$fontLegend=New-Object System.Drawing.Font('Malgun Gothic',27,[System.Drawing.FontStyle]::Regular,[System.Drawing.GraphicsUnit]::Pixel)
...
$colors=@('#2166ac','#4393c3','#3f9b78','#b3a329','#e48331','#bd2d45')
$left=160; $right=1860; $top=145; $bottom=665
...
for ($i=0;$i -lt 6;$i++) {
    ...
    $lx=270+($i%3)*540; $ly=30+[math]::Floor($i/3)*46
...
DS '시간 (KST)' 920 733 $fontAxis
$g.TranslateTransform(35,587);$g.RotateTransform(-90)
DS 'MAE 개선량 (보정 전 − 보정 후, °C)' 0 0 $fontAxis
$g.ResetTransform()
$imagePath=Join-Path $out '16_hourly_mae_improvement_altitude_span_stations.png'
$bmp.Save($imagePath,[System.Drawing.Imaging.ImageFormat]::Png)
$g.Dispose();$bmp.Dispose()
$manifest=[ordered]@{COHORT_N=$stations.Count;TARGET_ELEVATIONS_M=$targetElevations;SELECTED_N=6;HOURLY_ROWS=$rows.Count;ALL_MAX_ELEV_M=[double]$stations[-1].elev_m;SELECTED_MAX_ELEV_M=($selection.ELEV_M|Measure-Object -Maximum).Maximum;INPUT_SHA256=@((Get-FileHash $cohortPath).Hash,(Get-FileHash $metricsPath).Hash);SELECTION='50/300/600/900/1200/1500m 최근접, 동률이면 낮은 고도·작은 지점번호';NOTE='실제 관측소 사례이며 집단 성능 대표성을 보장하지 않음'}
