...
from analysis import analyze_station_bias as bias_analysis
from analysis.analyze_station_bias import run_analysis
...
def mocked_full_inputs(root, fail_figure=False):
    ...
    with ExitStack() as stack:
        stack.enter_context(
            patch('analysis.analyze_station_bias.open_readonly_db', return_value=connection)
        )
        stack.enter_context(
            patch(
                'analysis.analyze_station_bias.load_analysis_station_ids',
                return_value=analyzed_ids,
            )
        )
        stack.enter_context(
            patch(
                'analysis.analyze_station_bias._collect_analysis_frames',
                return_value=(station.copy(), monthly.copy(), diurnal.copy()),
            )
        )
        stack.enter_context(
            patch(
                'analysis.analyze_station_bias._merge_full_population_features',
                side_effect=lambda frame: frame,
            )
        )
        stack.enter_context(
            patch('analysis.analyze_station_bias._build_evidence', return_value=evidence)
        )
        stack.enter_context(
            patch('analysis.analyze_station_bias.EXCLUDED_PATH', excluded_path)
        )
        stack.enter_context(
            patch('analysis.analyze_station_bias.CANDIDATE_PATH', candidate_path)
        )
        ...
        for function, filename in zip(figure_functions, figure_names):
            ...
            stack.enter_context(
                patch(f"analysis.analyze_station_bias.{function}", side_effect=side_effect)
            )
...
class BiasAnalysisIntegrationTests(unittest.TestCase):
        ...
        for enforce_population in [False, True]:
            for stations, message in [
                ...
                with self.subTest(
                    ...
                    with patch(
                        'analysis.analyze_station_bias.open_readonly_db',
    ...
    def test_unknown_station_errors_in_both_population_modes(self):
        ...
        with ExitStack() as stack:
            ...
            stack.enter_context(
                patch('analysis.analyze_station_bias.load_neighbor_map', return_value={})
            )
            stack.enter_context(
                patch('analysis.analyze_station_bias.load_station_meta', return_value=pd.DataFrame())
            )
            stack.enter_context(
                patch(
                    'analysis.analyze_station_bias.load_station_pairs',
        ...
        connection = Mock()
        with patch(
            'analysis.analyze_station_bias.open_readonly_db', return_value=connection
        ), patch(
            'analysis.analyze_station_bias.load_analysis_station_ids', return_value=["116"]
        ):
