...
from find_stations import get_distance_km
from bias_physical_followup import (
...
try:
    runner = importlib.import_module("analyze_bias_physical_followup")
...
class CanonicalAndArtifactTests(RunnerFunctionTestCase):
    ...
    def test_output_guard_and_production_api_fail_before_any_unsafe_write_or_db_load(self):
        ...
        with _workspace_temp_dir() as temp_dir:
            ...
            with self.assertRaises(ValueError):
                guard(unsafe)
            with patch("bias_data.open_readonly_db") as db_open:
                with self.assertRaises(ValueError):
...
class CriticalPreflightRegressionTests(RunnerFunctionTestCase):
    ...
    def test_production_source_path_streams_one_station_and_counts_actual_queries_reads(self):
        ...
        with _workspace_temp_dir() as temp_dir:
            ...
            with (
                patch("bias_data.open_readonly_db", return_value=connection),
                patch("bias_data.load_neighbor_map", return_value=fixture["neighbor_map"]),
                patch("bias_data.load_station_pairs", return_value=fixture["pairs"]) as load_pairs,
                patch("find_stations.load_station_meta", return_value=fixture["meta"]),
                patch.object(
                    runner,
            ...
            with (
                patch("bias_data.open_readonly_db", return_value=failed_connection),
                patch("bias_data.load_neighbor_map", return_value=fixture["neighbor_map"]),
                patch("bias_data.load_station_pairs", return_value=fixture["pairs"]),
                patch("find_stations.load_station_meta", return_value=fixture["meta"]),
                patch.object(
                    runner,
                    "validate_canonical_station_results",
...
class IntegrationReviewFixRound1Tests(RunnerFunctionTestCase):
    ...
    def test_i4_two_targets_share_physical_aws_reads_through_bounded_cache(self):
        ...
        with _workspace_temp_dir() as temp_dir:
            ...
            with (
                patch("bias_data.open_readonly_db", return_value=connection),
                patch("bias_data.load_neighbor_map", return_value=neighbor_map),
                patch("bias_data.load_station_pairs", side_effect=load_pairs),
                patch("find_stations.load_station_meta", return_value=fixture["meta"]),
                patch.object(
                    runner,
                    "validate_canonical_station_results",
    ...
    def test_i1_wal_source_is_rejected_before_production_loader(self):
        ...
        with _workspace_temp_dir() as temp_dir:
            ...
            with (
                ...
                patch("bias_data.open_readonly_db") as loader,
