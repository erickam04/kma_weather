...
실행:
    python run_pipeline.py                # 전체 (batch는 12개월 로드라 오래 걸림)
    python run_pipeline.py --only batch   # 한 단계만
    python run_pipeline.py --skip batch   # batch 제외 (기존 raw 재사용해 분석/시각화만)
...
def step_viz_batch():
    import visualize_batch
    visualize_batch.main()


def step_viz_targets():
    import run_visualize_targets
    run_visualize_targets.run()


def step_viz_maps():
    import visualize_targets_map
